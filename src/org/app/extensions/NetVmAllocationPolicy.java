 ...
 * @since       1.0
 */
public class NetVmAllocationPolicy extends VmAllocationPolicySimple implements VmAllocationPolicyExtensible {
    ...
    public NetVmAllocationPolicy(Map<Integer,Integer> vmforHost,int simulationId,List<? extends NetPowerHost> list, double utilizationThreshold) {
        super(list);
...
//    }
    ...
    public NetPowerHost findHostForVm(Vm vm) {
    	double minPower = Double.MAX_VALUE;
    	 NetPowerHost allocatedHost = null;
    	 Map<Integer,Integer> tmpvmMap=CloudsimConstants.vmIdMap.get(simulationId);
    	 Map<Integer,Integer> tmphostMap=CloudsimConstants.hostIdMap.get(simulationId);
		
    	 Iterator<Integer> it=tmpvmMap.keySet().iterator();
		 int oldVmid=0;
		 while(it.hasNext()){
			 int tmpvmid=it.next();
			 if(tmpvmMap.get(tmpvmid)==vm.getId()){
				 oldVmid=tmpvmid;
				 break;
			 }
		 }
	     int oldHostid=vmforHost.get(oldVmid);
		 int newHostid=tmphostMap.get(oldHostid);    
		 
    	 for (NetPowerHost host : this.<NetPowerHost>getHostList()) {    		 
    		 if(host.getId()==newHostid){
               if (host.isSuitableForVm(vm)) {
               double maxUtilization = getMaxUtilizationAfterAllocation(host, vm);
               if ((!vm.isBeingInstantiated() && maxUtilization > getUpperUtilizationThreshold()) || (vm.isBeingInstantiated() && maxUtilization > 1.0)) {
                   continue;
    ...
    protected static double getHostRamUtilization(NetPowerHost host) {
        double ramUtilization =  host.getRamProvisioner().getUsedRam();
